...
// its creating "VkDebugUtilsMessengerEXT" object
...
// its destroying "VkDebugUtilsMessengerEXT" object
...
// movement variables
#pragma region	
glm::vec3 lookFrom = glm::vec3(2.0f, 0.0f, 0.0f);
glm::vec3 lookAt = glm::vec3(0.0f, 0.0f, 0.0f);
glm::vec3 verticalWorldAxis = glm::vec3(0.0f, 1.0f, 0.0f);
glm::vec3 cameraDirection = glm::normalize(lookAt - lookFrom);
...
private:
	...
	void initWindow()
	{
		...
		window = glfwCreateWindow(WIDTH, HEIGHT, "Vulkan and RTX", nullptr, nullptr);
	...
	{
		if (verticalFov >= 1.0f && verticalFov <= 78.0f)
			verticalFov -= yoffset;
		if (verticalFov <= 1.0f)
			verticalFov = 1.0f;
		if (verticalFov >= 78.0f)
			verticalFov = 78.0f;
	}
	...
	void movePerson()
	{
		...
		auto deltaTime = std::chrono::duration<float, std::chrono::seconds::period>(currentTime - previousTime).count() / 200000;
		float cameraSpeed = 1.00f * deltaTime;
		...
		if (keys[GLFW_KEY_W])
		{
			lookFrom += cameraSpeed * cameraDirection;
		}
		...
		if (keys[GLFW_KEY_A])
		{
			lookFrom -= glm::normalize(glm::cross(cameraDirection, verticalWorldAxis)) * cameraSpeed;
		}
		...
		{
			lookFrom += verticalWorldAxis * cameraSpeed * 0.7f;
		}
	...
	void generateCubicLandscape(size_t worldWidth, size_t worldLenght)
	{
		for (size_t i = 0; i < worldWidth; i++)
		{
			for (size_t j = 0; j < worldLenght; j++)
			{
				float random_height = 0.01 * (rand() % 51);
				generateCube(0.0f + (float)i * 0.5f - worldWidth / 4, 0.0f + (float)j * 0.5f - worldLenght / 4, -2 + random_height);
			}
		}
	}
	...
	void generateCube(float x, float y, float z)
	{
		...
		std::vector <Vertex> localVertices(16);
		...
		localVertices[12].pos = { glm::vec3(0.0f, 0.0f, 0.0f) + glm::vec3(x, y, z) };// 0
		localVertices[13].pos = { glm::vec3(0.5f, 0.0f, 0.0f) + glm::vec3(x, y, z) };// 1
		localVertices[8].pos = { glm::vec3(0.5f, 0.5f, 0.0f) + glm::vec3(x, y, z) };// 2
		localVertices[9].pos = { glm::vec3(0.0f, 0.5f, 0.0f) + glm::vec3(x, y, z) };// 3
		localVertices[14].pos = { glm::vec3(0.0f, 0.0f, 0.5f) + glm::vec3(x, y, z) };// 4
		localVertices[15].pos = { glm::vec3(0.5f, 0.0f, 0.5f) + glm::vec3(x, y, z) };// 5
		localVertices[10].pos = { glm::vec3(0.5f, 0.5f, 0.5f) + glm::vec3(x, y, z) };// 6
		localVertices[11].pos = { glm::vec3(0.0f, 0.5f, 0.5f) + glm::vec3(x, y, z) };// 7
		...
		localVertices[8].texCoord = { glm::vec2(1.0f, 1.0f) };
		localVertices[9].texCoord = { glm::vec2(0.0f, 1.0f) };
		localVertices[14].texCoord = { glm::vec2(1.0f, 0.0f) };
		localVertices[15].texCoord = { glm::vec2(0.0f, 0.0f) };
		localVertices[10].texCoord = { glm::vec2(0.0f, 1.0f) };
		localVertices[11].texCoord = { glm::vec2(1.0f, 1.0f) };

		uint32_t verticesNum = static_cast<uint32_t>(vertices.size());

		std::vector<uint32_t> localIndices = {
			0 + verticesNum, 3 + verticesNum, 1 + verticesNum,
			3 + verticesNum, 2 + verticesNum, 1 + verticesNum,
			5 + verticesNum, 6 + verticesNum, 4 + verticesNum,
			6 + verticesNum, 7 + verticesNum, 4 + verticesNum,
			3 + verticesNum, 7 + verticesNum, 2 + verticesNum,
			7 + verticesNum, 6 + verticesNum, 2 + verticesNum,
			4 + verticesNum, 0 + verticesNum, 5 + verticesNum,
			0 + verticesNum, 1 + verticesNum, 5 + verticesNum,

			12 + verticesNum, 14 + verticesNum, 11 + verticesNum,
			12 + verticesNum, 11 + verticesNum, 9 + verticesNum,
			10 + verticesNum, 15 + verticesNum, 13 + verticesNum,
			10 + verticesNum, 13 + verticesNum, 8 + verticesNum
		};
	...
	//// returning byte size of vertices
	//void getVerticesDataSize()
	//{
	//	for (size_t i = 0; i < modelsVertices.size(); i++)
	//	{
	//		verticesDataSize += sizeof(Vertex) * modelsVertices[i].size();
	//	}
	//}

	//// returning byte size of indices
	//void getIndicesDataSize()
	//{
	//	for (size_t i = 0; i < modelsIndices.size(); i++)
	//	{
	//		indicesDataSize += sizeof(modelsIndices[0][0]) * modelsIndices[i].size();
	//	}
	//}
	...
	void recordCommandBuffer(VkCommandBuffer commandBuffer, uint32_t imageIndex)
	{
		...
		/*for (size_t i = 0; i < descriptorSets.size(); i++)
		{
			...
			{
				vkCmdDrawIndexed(commandBuffer, static_cast<uint32_t>(modelsIndices[j].size()), 1, 0, 0, 0);
			}
		}*/
	...
	// creating swap chain with the best properties
	...
	void createRenderPass()
	{
		...
		colorAttachment.samples = msaaSamples;// samples per fragment		
	...
	// the region of the framebuffer that the output will be rendered to
			...
			// filtering the pixel that was taken from the rasterizer, in our case its all pixel
			VkRect2D scissor{};
			scissor.offset = { 0, 0 };
	...
	void updateUniformBuffer(uint32_t currentImage)
	{
		...
		ubo.view = glm::lookAt(lookFrom, lookFrom + cameraDirection, verticalWorldAxis);

		ubo.proj = glm::perspective(glm::radians(verticalFov), swapChainExtent.width / (float)swapChainExtent.height, 0.1f, 40.0f);
		ubo.proj[1][1] *= -1;
